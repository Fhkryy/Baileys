...
export const makeNewsletterSocket = (config) => {
  ...
  const { delay, query, generateMessageTag, groupAcceptInvite } = sock;
  ...
  const newsletterQuery = loadBase(newsletterWMexQuery, QueryIds, groupAcceptInvite);
  ...
  return {
    ...
    newsletterChangeOwner: async (jid, newOwnerJid) => {
      await newsletterWMexQuery(jid, QueryIds.CHANGE_OWNER, { user_id: newOwnerJid });
    },
    /** user is Lid, not Jid */
    newsletterPromote: async (jid, userJid) => {
      await newsletterWMexQuery(jid, QueryIds.PROMOTE, { user_id: userJid });
    },
    newsletterDemote: async (jid, userJid) => {
      await newsletterWMexQuery(jid, QueryIds.DEMOTE, { user_id: userJid });
    },
